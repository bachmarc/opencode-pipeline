...
def test_requirements_no_req_definitions():
    ...
    # e.g., "REQ-001 Two-clone topology:" or "REQ-002 Retro traceability:"
...
def test_design_no_numbered_req_sections():
    """design.md must NOT contain old §-numbered sections with REQ-IDs (e.g., '## 1. Two-clone topology (REQ-001)')."""
    design_file = Path(__file__).parent.parent / "docs" / "design.md"
    assert design_file.exists(), "docs/design.md not found"
    
    content = read_file_content(design_file)
    
    # Pattern: ## <digit>. <title> (REQ-<digits>)
    # e.g., "## 1. Two-clone topology (REQ-001, REQ-001a, REQ-005)"
...
def test_design_has_decisions_table():
    ...
    # Check for Decisions table header and at least one D row
