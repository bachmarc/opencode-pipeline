...
- IF $ARGUMENTS empty and implementation running: act as status dialogue partner — use `scripts/pipeline_status.py` to aggregate status, summarize: `STORIES.md` status, all `feature/*` branches, latest `pytest` logs, open FAILs/BLOCKEDs (table story|branch|tests|qa|error) and answer user question about status/errors
